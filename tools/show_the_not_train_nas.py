...
if __name__ == '__main__':
    ...
    # trained_algorithm_list = []
    # algorithm_list = []

    algorithm_list_stats_dict = dict()
    algorithm_list_stats = list()
    ...
    for root, dirs, files in os.walk('configs'):
        ...
        if files.__len__()>0:   
            for config in files:
                if '.md' not in config and '.yml' not in config:
                    config_name = root+'/'+config
                    infos = config_name.split('/')
                    dataset = infos[1]
                    algorithm = infos[2]
                    config_algorithm = infos[3]
                    config_name = config_name

                    algorithm_list_stats_dict[config_name] = dict(
                        config=config_algorithm,
                        dataset=dataset,
                        algorithm=algorithm,
                        epoch=0,
                        eval_epoch=0,
                        pkl_epoch=0,
                        cmd='CUDA_VISIBLE_DEVICES=1 python tools/train.py '+config_name +' --auto-resume',
                        test_cmd = ''
                        )
    ...
    for root, dirs, files in os.walk('work_dirs_no_pth'):
        ...
        if files.__len__() > 0:
            ...
            config_name = root + '.py'
            config_name = config_name.replace('work_dirs_no_pth', 'configs')
            infos = config_name.split('/')
            ...
            algorithm_list_stats_dict[config_name]['test_cmd'] = 'CUDA_VISIBLE_DEVICES=1 python tools/test_batch.py --work_dirs ' + root
            



    for k,v in algorithm_list_stats_dict.items():

        algorithm_list_stats.append(v)
        
    # print(algorithm_list_stats)
...
df.to_excel(prefix+'trained_origin.xlsx')
